function validateForm(form) {
  ...
  function validarInicio() {
    var subtipo = form.getValue("subtipo");
    var numSolicitacao = form.getValue("numSolicitacao");
    var problema = form.getValue("solicitacao");

    if (subtipo == "FLUIG" && numSolicitacao == "") {
      throw "Você precisa informar o número da solicitação do seu problema!";
    }
    ...
    if (
      problema == "NÃO CONSIGO COMPLETAR SOLICITAÇÃO" ||
      problema == "ITEM/NF NÃO APARECE"
    ) {
  ...
  function validarLiberacao() {
    ...
    if (liberacao != "Liberado") {
      throw (
        "O pedido está " +
        liberacao +
        ", mas deve estar liberado para o processo continuar."
      );
    }
  }

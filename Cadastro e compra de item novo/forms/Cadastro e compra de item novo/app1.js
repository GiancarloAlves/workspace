...
function taskHandler() {
  ...
  switch (TASK) {
    ...
    case EVENTO_INICIO:
      ...
      $(`[data-cod]`).hide();
      $(`[data-follow]`).hide();
      ligarLED("led_cadastro");
      $(`#cadastro`).removeClass("blocked");
      $(`#cadastro`).removeClass("in");
      dispararAlerta({
        icone: "warning",
        titulo: "Atenção!",
        mensagem: `Anexar orçamento do fornecedor e OC e/ou aceite do cliente.`,
      });
      console.log(document.getElementById("matcomprador").value);
      break;
    ...
    case CADASTRO:
      ...
      $(`#cadastro`).removeClass("blocked");
      $(`#cadastro`).removeClass("in");
      $(`.sectionDadosCorretos`).removeClass("hidden");
      break;
    ...
    case CONFERENCIA_COMPRADOR:
      updateLED("led_conferencia_cadastro");
      ligarLED("led_conferencia_cadastro");
      tratarPainel("conferencia_cadastro");
      break;
    ...
    case LIBERACAO_PEDIDO:
      ...
      $(`[data-prod]`).hide();
      $(`[data-follow]`).show();
      const status_lib = document.getElementsByClassName("status2");
...
function setSelectedZoomItem(selectedItem) {
  ...
  switch (inputId) {
    ...
    case `codfornec`:
      ...
      $(`#compras1`).val(selectedItem.NOME);
      $(`#matcompras1`).val(selectedItem.CODCOMPRADOR);
      $("#additem").removeAttr("hidden");
      break;
...
function validaStatus() {
  ...
  if (STATUS !== "Pendente") {
    dispararAlerta({
      icone: "warning",
      titulo: "Atenção!",
      mensagem: `Pedido possui status diferente de pendente ou liberado. Só é possível continuar se o pedido estiver pendente ou liberado.`,
    });
  }
}
...
function contarProduto() {
  ...
  var indexes = Array.from($(":input[id*=produto___]")).map((el) => {
    return el.id.split("___")[1];
  });

  $(`#quantItens`).val(indexes.length);
}

function copiaTabela() {
  const cadastro = Array.from(
    document.querySelectorAll("input[id*=produto___]")
  ).map((input) => input.value);
  const codprod = Array.from(
    document.querySelectorAll("input[id*=codprod___]")
  ).map((input) => input.value);
  ...
  for (var i = 0; i < cadastro.length; i++) {
    const produto = cadastro[i];
    const codigo = codprod[i];

    wdkAddChild("tabFollow");

    $("#produtoFollow___" + newId).val(produto);
    $("#codprodFollow___" + newId).val(codigo);
  }
}

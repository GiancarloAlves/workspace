...
function atualizaValores(){
    ...
    var dataset = DatasetFactory.getDataset("ds_cancelamento_pedido_form_winthor", null, constraints, null);
    console.log(dataset)
...
function validaStatus(){
    ...
    if(STATUS !== "Aguardando Onda") {
        dispararAlerta({icone: "warning", titulo: "Atenção!", mensagem: `Pedido já passou da etapa de separação! Ao continuar com a solicitação, ela será enviada ao faturamento. A nota será emitida e o processo de cancelamento de NF será disparado automaticamente.`})
    } else { dispararAlerta({icone: "warning", titulo: "Atenção!", mensagem: `Pedido está aguardando onda. Se a separação iniciar, será necessário aguardar o faturamento da NF.`})

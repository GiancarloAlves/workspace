...
function validarFollow(){
    
    var follow = form.getValue("recebido");
    if (follow == "nao") {
        throw("Você marcou que o produto não foi recebido!");
    }

}

function validarRecebimento() {
    var recebimento = form.getValue("rdRecebido");
    if (recebimento == "" || recebimento == "nao") {
        throw ("Você marcou que o produto não foi recebido!");
    }
}
  ...
  function validarPaiFilhoA() {
    ...
    if(indexes.length > 0) {
        for(var i = 0; i < indexes.length; i++) { 
            ...
            if(form.getValue("codfornec___" + indexes[i]) == null || form.getValue("codfornec___" + indexes[i]) == "") {
                throw "<strong>Preencha a coluna Referência da Marca!</strong>"; }

            if(form.getValue("fornec___" + indexes[i]) == null || form.getValue("fornec___" + indexes[i]) == "") {
            ...
            if(form.getValue("icms___" + indexes[i]) == null || form.getValue("icms___" + indexes[i]) == "") {

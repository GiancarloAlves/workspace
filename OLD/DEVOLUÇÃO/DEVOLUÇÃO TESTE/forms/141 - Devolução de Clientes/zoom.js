function setSelectedZoomItem(selectedItem) {
    ...
    const row = selectedItem.inputId.split('___')[1]
    ...
    switch (inputId) {
        ...
        case `descProduto___${row}`:
            reloadZoomFilterValues(`codProduto___${row}`, "CODPROD," + selectedItem.CODPROD)
            break  
    }
}

function limparPaiFilho() {
    Array.from($(`${TABLE__GRID_ID} tbody tr:gt(0)`)).map(e => fnWdkRemoveChild(e))
}
